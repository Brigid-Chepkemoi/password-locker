...
class Credentials(object):
    ...
    def create_credentials(self, credentials_list):
        if credentials_list.__contains__(self.created_by+'\t \t'+self.username+'\t \t'+self.password+'\t \t'+self.platform):
            print("Account already exists")
        else:
            credentials_list.append(self.created_by+' '+self.username+' '+self.password+' '+self.platform)
            handle = open("/home/brie/dat2.txt", "a")
            handle.write(self.created_by+'\t \t'+self.username+'\t \t'+self.password+'\t \t'+self.platform + '\n')
            handle.close()
    ...
    def delete_credentials(self, authenticated_user, account, platform):
        ...
        for line in input_file:
            if not line.lstrip().startswith(authenticated_user+"\t \t"+account) and not line.lstrip().endswith(platform):
